...
class OrlenGasConfigFlow(ConfigFlow, domain="orlen_gas_sensor"):
    """Example config flow."""
    ...
    async def async_step_user(self, user_input: Optional[Dict[str, Any]] = None):
        ...
        if user_input is not None:
            api = OrlenApi(user_input[CONF_USERNAME], user_input[CONF_PASSWORD])
            try:
                await self.hass.async_add_executor_job(api.login)
                return self.async_create_entry(title="Orlen sensor", data=user_input)
