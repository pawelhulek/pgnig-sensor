...
async def async_setup_entry(
    ...
    for x in pgps.ppg_list:
        ...
        async_add_entities(
            [OrlenSensor(hass, api, meter_id, x.id_local),
             OrlenInvoiceSensor(hass, api, meter_id, x.id_local),
             OrlenCostTrackingSensor(hass, api, meter_id, x.id_local)],
...
async def async_setup_platform(
    ...
    api = OrlenApi(config.get(CONF_USERNAME), config.get(CONF_PASSWORD))
    try:
        pgps = await hass.async_add_executor_job(api.meterList)
    except Exception:
        raise ValueError
    ...
    for x in pgps.ppg_list:
        async_add_entities(
            [OrlenSensor(hass, api, x.meter_number, x.id_local),
             OrlenInvoiceSensor(hass, api, x.meter_number, x.id_local),
             OrlenCostTrackingSensor(hass, api, x.meter_number, x.id_local)],
            update_before_add=True)


class OrlenSensor(SensorEntity):
    def __init__(self, hass, api: OrlenApi, meter_id: string, id_local: int) -> None:
    ...
    def unique_id(self) -> str | None:
        return "orlen_sensor_" + self.meter_id + "_" + str(self.id_local)

    @property
    def device_info(self):
        return {
            "identifiers": {("orlen_gas_sensor", self.meter_id)},
...
class OrlenInvoiceSensor(SensorEntity):
    def __init__(self, hass, api: OrlenApi, meter_id: string, id_local: int) -> None:
    ...
    @property
    def unique_id(self) -> str | None:
        return "orlen_invoice_sensor_" + self.meter_id + "_" + str(self.id_local)
...
class OrlenCostTrackingSensor(SensorEntity):
    def __init__(self, hass, api: OrlenApi, meter_id: string, id_local: int) -> None:
    ...
    @property
    def unique_id(self) -> str | None:
        return "orlen_cost_tracking_sensor_" + self.meter_id + "_" + str(self.id_local)
