...
def add_entry(
    hass: HomeAssistant, auth_method: str = AUTH_METHOD_ORLEN_ID
) -> MockConfigEntry:
...
async def test_session_refresh_is_scheduled_only_for_orlen_id(
    ...
    entry = add_entry(hass, auth_method=auth_method)
    with (
        patch("custom_components.pgnig_gas_sensor.PgnigApi", return_value=mock_api),
        patch.object(hass.config_entries, "async_forward_entry_setups", AsyncMock()),
        patch(
...
async def test_background_refresh_starts_reauth_flow(
    ...
    hass.data[DOMAIN] = {
        entry.entry_id: PgnigRuntimeData(api=mock_api, meters=mock_meters)
    }
